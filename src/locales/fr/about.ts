export default {
    aboutDescription: "Ce site web à but non lucratif fournit des données UV (via l'API open source <openuvLink>OpenUV</openuvLink>) pour la zone géographique de votre choix. Vous pouvez accéder aux informations UV de deux manières :",
    aboutDescriptionCoords: "<strong>Par coordonnées :</strong> saisissez la latitude et la longitude (en degrés décimaux), ainsi que l'altitude (facultative, en mètres). Vous pouvez facilement trouver vos coordonnées sur <gpsLink>Coordonnées GPS</gpsLink> et vérifier votre altitude à l'aide de <elevationLink>FreeMapTools</elevationLink>.",
    aboutDescriptionCity: "<strong>Par ville :</strong> Commencez à taper le nom d'une ville (au moins 3 lettres), affinez si besoin, puis choisissez-la dans la liste - les coordonnées seront récupérées automatiquement.",
    aboutCheckUvLink: "Consulter l'indice UV",
    aboutGuidesLink: "Guides & Conseils",
    aboutGuidesTitle: "Brillez intelligemment : Guides de protection UV",
    aboutBackLink: "Revenir à la liste des guides & conseils",
    card: {
        Link: "En savoir plus ",
        understand: {
            title: "Comprendre l'indice UV",
            description: "Découvrez ce que mesure l'indice UV, comment il est catégorisé, et pourquoi il est essentiel pour votre sécurité au soleil.",
        },
        calculate: {
            title: "Comment est calculé l'indice UV",
            description: "Comprenez les facteurs scientifiques et atmosphériques utilisés pour calculer l'indice UV.",
        },
        tips: {
            title: "Conseils pour une exposition solaire sûre",
            description: "Apprenez des conseils pratiques pour profiter du soleil en toute sécurité et réduire les risques de dommages cutanés.",
        },
        sunscreen: {
            title: "Bien choisir sa crème solaire",
            description: "Découvrez comment sélectionner la meilleure crème solaire pour vous protéger efficacement des UV.",
        },
        longTerm: {
            title: "Effets à long terme de l'exposition aux UV",
            description: "Découvrez comment une exposition prolongée aux UV peut affecter votre peau, vos yeux et votre système immunitaire.",
        },
        vitaminD: {
            title: "Vitamine D et lumière du soleil",
            description: "Apprenez comment l'exposition au soleil aide votre corps à produire de la vitamine D, et comment en tirer les bénéfices tout en restant prudent",
        },
        winter: {
            title: "Se protéger du soleil en hiver",
            description: "Découvrez pourquoi la protection solaire reste importante pendant l'hiver et en conditions neigeuses.",
        },
        highUvZones: {
            title: "Zones à fort indice UV toute l'année",
            description: "Identifiez les régions du monde où l'indice UV reste élevé en continu, et comment s'y protéger efficacement.",
        }
    }
} as const